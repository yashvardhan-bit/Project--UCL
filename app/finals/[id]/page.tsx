  ...
  return (
    <div style={{ backgroundColor: "var(--color-navy)" }}>
      <div className="max-w-4xl mx-auto px-6 pt-24">
        <Link href="/seasons" className="text-xs tracking-widest uppercase inline-block mb-8" style={{ color: "var(--color-gold)" }}>
          ← All Seasons
        </Link>
        ...
        <div className="mb-12">
          ...
          <h1 className="text-5xl md:text-6xl font-bold text-white mb-4" style={{ fontFamily: "var(--font-bebas)" }}>
            {final.winner} vs {final.runner_up}
          </h1>
          <div className="inline-block px-4 py-2 border text-lg font-mono" style={{ borderColor: "var(--color-gold)", color: "var(--color-gold)" }}>
            {final.score}
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div className="p-6 border border-white/10" style={{ backgroundColor: "var(--color-surface)" }}>
          ...
          <div className="p-6 border border-white/10" style={{ backgroundColor: "var(--color-surface)" }}>
            ...
            <div className="space-y-2">
              {Object.entries(final.road_to_glory).map(([round, opponent]) => (
                <div key={round} className="flex justify-between py-2 border-b border-white/10">
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={role} className="flex gap-3">
                <span className="text-xs font-mono w-10" style={{ color: "rgba(255,255,255,0.4)" }}>
                  {role}
                </span>
                <div className="flex flex-wrap gap-2">
                  {players.map((player) => (
                    <span key={player} className="text-xs px-2 py-1 border border-white/10 text-white/80">
                      {player}
                    </span>
                  ))}
                </div>
        ...
        <div className="text-center mb-12">
          <Link href="/seasons" className="inline-block px-6 py-3 text-sm font-semibold tracking-widest uppercase transition-colors" style={{ backgroundColor: "var(--color-gold)", color: "var(--color-navy)" }}>
            ← Back to Seasons
          </Link>
        </div>
      </div>
